...
void loop() {
  ...
  if (m1_auto_active) {
    ...
    } else { // Pokud zbývá méně než 8 stupňů k cíly, tak se sníží rychlost na 500
  ...
  if (isCalibrating) {
    ...
    // FÁZE 1: Hlídání přesného odjezdu o 9975 pulzů z enkodéru
      ...
      if (reached_proti || reached_po) {
        ...
        updateM2State();
        m2_calib_stage = 2; 
        SerialBT.println(">>> M2 dosahla pozice 90° od senzoru. Zastavuji...");
      }
    }
  ...
  if (isCalibrating && !m1_auto_active && m2_calib_stage == 2 && m1_current_freq == 0 && m2_current_freq == 0) {
    ...
    SerialBT.println(">>> AS5304 vynulovan.");
  }
