...
const int M1_RAMP_MIN = 20;   
const int M1_RAMP_MAX = 50;   
const int M2_RAMP_MIN = 30;   
const int M2_RAMP_MAX = 200;  
const int SMOOTH_ZONE = 1000; 

// NOVINKA: Multiplikátor brzdění (čím vyšší číslo, tím agresivnější/rychlejší zastavení)
...
void setupMotory() {
  ...
  ledcAttach(M1_STEP_PIN, 1001, PWM_RESOLUTION); // Donutí ESP32 použít Časovač A
  ledcAttach(M2_STEP_PIN, 1002, PWM_RESOLUTION); // Donutí ESP32 použít Časovač B
...
void updateMotorRamp(int step_pin, int &current_freq, int target_freq, unsigned long &last_update, int min_step, int max_step) {
  if (current_freq != target_freq && millis() - last_update >= RAMP_INTERVAL) {
    ...
    if (current_freq < target_freq) {
      ...
      // ZDE JE APLIKOVÁN MULTIPLIKÁTOR PRO RYCHLEJŠÍ BRZDĚNÍ
...
void zpracujPrikazBT(char cmd) {
  ...
  extern bool m1_auto_active; // Přistoupení ke stavu chytrého polohování

  switch (cmd) {
    // --- M1: Kolébka ---
    case 's': case 'S': 
      m1_auto_active = false; // Manuální zásah zruší auto-polohování
      m1_velocity_target += 200; 
      updateM1State();
      SerialBT.printf("M1 (Kolebka): Cíl %d Hz (Smer %s)\n", abs(m1_velocity_target), m1_velocity_target >= 0 ? "LOW" : "HIGH");
      break;
    ...
    // --- M2: Platforma ---
    case 'd': case 'D': 
      m2_velocity_target += 500; 
      updateM2State();
      SerialBT.printf("M2 (Platforma): Cíl %d Hz (Smer %s)\n", abs(m2_velocity_target), m2_velocity_target >= 0 ? "LOW" : "HIGH");
      break;
    ...
    // --- ZASTAVENÍ ---
    case 'x': case 'X':
      m1_auto_active = false; // Tvrdé zastavení ruší všechno
